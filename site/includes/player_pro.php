...
(function () {
  ...
  /**
   * حاوية لا يفتحها المتصفح → نطلب التحويل فوراً.
   *
   * لا ننتظر فشل التشغيل ولا فحص الصوت: ملف MKV لا يُفتح إطلاقاً، فلا
   * تصل شفرة hls.js إليه ولا يُطلق أي حدث نتعلّق به. المشغّل الأصلي
   * يكتفي برسالة «يلزم تحويلها إلى MP4» ثم يقف. هنا نحوّلها فعلاً.
   */
  function checkDeadContainer() {
    if (restreamActive) return;
    var url = currentStreamUrl();
    if (!url || !DEAD_CONTAINER.test(url)) return;

    var sref = currentStream();
    if (sref.id <= 0) return;           // لا نعرف رقمه — لا شيء نطلبه

    audioWarned  = true;                // يمنع تحذير الصوت من التكرار فوقه
    restreamTries = 0;
    say('هذه الصيغة لا يفتحها المتصفح — جارٍ التحويل…');
    tryRestream(sref, 'container');
  }

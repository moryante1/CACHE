...
/* ══ تمييز «الطلب أكبر من الحدّ» عن «انتهاء الجلسة» ══
   حين يتجاوز حجم الطلب post_max_size يتخلّص PHP من جسمه **كاملاً**:
   لا الملف وحده بل كل الحقول ومنها csrf_token. فيفشل csrfValidate()
   وتظهر رسالة انتهاء الجلسة، بينما الجلسة سليمة والعلّة أن الصورة
   أكبر من الحدّ. والرسالة لا تكتفي بإخفاء السبب بل ترسل المستخدم إلى
   تسجيل الدخول مراراً بلا فائدة.

   والمفارقة أن الملف يعالج أخطاء الحجم معالجةً صحيحة أدناه
   (UPLOAD_ERR_INI_SIZE وأخواتها) — لكن ذلك الفحص يأتي بعد هذا،
   فلا يُبلَغ أبداً في هذه الحالة تحديداً: الجسم ضاع قبل أن يصل،
   فلا $_FILES ولا خطأ رفع أصلاً.

   ⚠ الترتيب لا يجوز عكسه: فحص CSRF يبقى أولاً حفاظاً على الأمان.
   ما نغيّره هو دقّة الرسالة عند الرفض لا شرط الرفض نفسه. */
if (!csrfValidate()) {
    $__cl = (int) ($_SERVER['CONTENT_LENGTH'] ?? 0);
    $__mp = trim((string) ini_get('post_max_size'));

    $__bytes = static function (string $v): int {
        $v = trim($v);
        if ($v === '') return 0;
    ...
    // الجسم وصل فارغاً رغم أن المتصفح أرسل حمولة ⇒ أسقطه PHP
    $__lost  = empty($_POST) && empty($_FILES) && $__cl > 0;
    $__over  = $__max > 0 && $__cl > $__max;

    if ($__over || $__lost) {
        logoFail(
            'الصورة أكبر ممّا يسمح به الخادم فرُفض الطلب كاملاً — '
            . 'الحدّ: ' . ($__mp !== '' ? $__mp : 'غير معروف')
    ...
    logoFail('انتهت صلاحية الجلسة أو رمز الحماية غير مطابق — حدّث الصفحة وأعد المحاولة.', 403);
}
